    ...
    <div>
        <ToolBar>
            ...
            <div class="animated bounceInLeft">
                <span
                    ...
                    @click="clickTheme(index,theme)"
                    :class="{'spanActive':currentTheme===index}"
                >
                    <el-button size="small" round>{{theme}}</el-button>&nbsp;
                </span>
            </div>
        ...
        <div class="animated fadeInUp">
            <el-table :data="tableDataSentences" border height="485px" style="width: 100%">
                ...
                <el-table-column prop="sentence_date" label="发布日期" width="180"></el-table-column>
...
import { sentenceTheme as sentenceThemeApi } from "../../api/content";
...
export default {
    ...
    methods: {
        ...
        sentenceEditChange() {},
        clickTheme(index, theme) {
            this.currentTheme = index;
            console.log(theme);
            console.log(this.currentTheme);
            sentenceThemeApi({
                sentence_theme: this.themeList[this.currentTheme]
            })
                .then(res => {
                    console.log("得到《"+this.themeList[this.currentTheme]+"》的半句res");
                    console.log(res)
                    this.tableDataSentences = res;
                })
                .catch(res => {
                    console.log("没有拿到半句主题");
                });
        },
        handleClick(row) {
            this.$alert(row, "半句名称", {
        ...
        deleteSentence(row) {
            let self = this;
            self.$confirm(
                "此操作将删除该半句(不只是从主题中删除，从数据库中也删除，若需更改主题，请点击编辑按钮), 是否继续?",
                "提示",
                {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
        ...
        getAllSentenceTheme(theme) {
            sentenceThemeApi({
                sentence_theme: ""
            })
                .then(res => {
                    console.log("得到所有主题的半句res");
                    
                    for (const index in res) {
                        // console.log(res[index].sentence_theme);
                        if (res[index].sentence_theme)
                            this.themeList.push(res[index].sentence_theme);
                    }
                    this.themeList = uniqueArray(this.themeList);
                })
                .catch(res => {
                    console.log("没有拿到半句主题");
                });
